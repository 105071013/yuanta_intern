...
Factor_args[Main_Factor_Name]['factors'] = [Main_Factor_Name +"_"+str(periods)]
...
reader = pd.read_csv('./Data/output/df_merge_3.txt', chunksize=10 ** 6)
df_factor = pd.concat([x for x in reader], ignore_index=True)
df_merge_final        = pd.merge(left=df_merge_final,right=df_factor[['Date','Code',Main_Factor_Name]],on=['Date','Code'],how='left')
#df_merge_final[Main_Factor_Name] = df_merge_final[Main_Factor_Name]/df_merge_final['sharecapital']
df_merge_final[factor] = df_merge_final.groupby('Code',as_index=False)[Main_Factor_Name].rolling(periods,min_periods=periods).sum().reset_index(drop=True,level=0)
